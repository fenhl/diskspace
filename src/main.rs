...
struct Args {
    ...
    #[clap(long)]
    #[cfg_attr(windows, clap(hide = true))]
    files: bool,
    ...
    #[cfg_attr(windows, clap(default_value = "C:/"))]
    #[cfg_attr(not(windows), clap(default_value = "/"))]
    path: PathBuf,
}
...
fn main(args: Args) -> io::Result<()> {
    ...
    let low_space = fs.avail < min_space || (fs.avail.as_u64() as f64 / fs.total.as_u64() as f64) < min_fraction;
    let low_files = {
        #[cfg(windows)] { false }
        #[cfg(not(windows))] {
    ...
    if !low_space && !low_files { return Ok(()) }
    if args.quiet {
        exit(1);
